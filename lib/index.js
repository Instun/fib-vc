...
function ldsign_issue({ ld, type, id, key, date, proofPurpose }) {
    check_type(ld, type);
    ...
    var data = Buffer.concat([
        ...
        hash.sha256(jsonld.canonize(ld, {
            documentLoader: jsonld._documentLoader
        })).digest()
    ]);
...
function ldsign_verify({ ld, type, key }) {
    try {
        check_type(ld, type);
...
function credential_issue({ credential, id, key, date, proofPurpose }) {
    return ldsign_issue({ ld: credential, type: "VerifiableCredential", id, key, date, proofPurpose });
}

function credential_verify({ credential, key }) {
    return ldsign_verify({ ld: credential, type: "VerifiableCredential", key });
}

function presentation_issue({ presentation, id, key, date, proofPurpose }) {
    return ldsign_issue({ ld: presentation, type: "VerifiablePresentation", id, key, date, proofPurpose });
}

function presentation_verify({ presentation, key }) {
    return ldsign_verify({ ld: presentation, type: "VerifiablePresentation", key });
}
